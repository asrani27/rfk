...
{
    public function subById($program_id, $kegiatan_id)
    {
        $data = Subkegiatan::where('kegiatan_id', $kegiatan_id)->orderBy('id', 'DESC')->get()->map(function ($item) {
    ...
    public function delete($program_id, $kegiatan_id, $subkegiatan_id)
    {
        try {
            ...
            toastr()->error('Tidak bisa di hapus karena memiliki Uraian kegiatan');
            return back();
        }
    }
}
